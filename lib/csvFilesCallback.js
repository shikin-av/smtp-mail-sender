...
const parseFileString = require('./parseFileString')
...
const csvFilesCallback = ({ csvFiles, emails }) => {
	...
	for(let i=0; i<csvFiles.length; i++){
		...
		fs.createReadStream(`${__dirname}/../${CONFIG.CSV_FOLDER}/${csvFiles[i]}`)
			.pipe(csv())
			.on('data', row => emails.push(parseFileString(row)))
			.on('end', (data) => {
				fileFinished++;
				...
				if(fileFinished==csvFiles.length){	
					...
					console.log('Чтение из ' + csvFiles.length + ' файлов завершено. Всего ' + emails.length + ' организаций');
					// console.log(emails)
				}
